...
from transformers import (PreTrainedTokenizerFast, BertForMaskedLM, 
                          ...
                          RoFormerModel,RoFormerConfig,
                          RoFormerTokenizerFast, RoFormerForMaskedLM)
...
def main(
    ...
    # Load tokenizer
    # Correspond with PreTrainedTokenizerFast
    # RoFormerTokenizerFast inherits from PreTrainedTokenizerFast which contains most of the main methods
    tokenizer = RoFormerTokenizerFast.from_pretrained(tokenizer_path)
    ...
    # Initializing a RoFormer Model with a language modeling head on top (with random weights)
    # Correspond with BertForMaskedLM
    configuration = RoFormerConfig()
    configuration.rotary_value = True
    model = RoFormerForMaskedLM(configuration).to(device) 
    #model = RoFormerForMaskedLM.from_pretrained(pretrained_model).to(device)
